...
The easiest way to use this tool is via the interactive dashboard:

1. Make sure you have installed the requirements, which now includes Streamlit.
2. Run the application from your terminal:
   ```bash
   streamlit run app.py
   ```
3. A browser window will automatically open with a user-friendly dashboard where you can configure the target phone number, batch sizes, concurrent browsers, and start/stop the attack while monitoring live logs and progress.

### Using the Terminal

If you prefer to run the script directly from the terminal, open `sms_bomber.py` in a text editor and modify these variables at the top of the file:
